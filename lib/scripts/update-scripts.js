...
fs.readdirSync(path.resolve('src', 'styles', 'colorschemes')).forEach((c) => {
    ...
    scripts[`serve:${scheme}`] = `set NODE_OPTIONS='--scheme false' && webpack serve --config ./node_modules/@caweb/webpack/webpack.config.js ./lib/entry/${scheme}.js ./lib/scripts/webpack.test.js --merge`;
    scripts[`serve:${scheme}:quick`] = `set NODE_OPTIONS='--no-jshint --no-audit --no-a11y --scheme false' && webpack serve --config ./node_modules/@caweb/webpack/webpack.config.js ./lib/entry/${scheme}.js ./lib/scripts/webpack.test.js --merge`;
